...
var EntityMetadataBuilder = (function () {
    ...
    EntityMetadataBuilder.prototype.build = function (entityClasses) {
        ...
        entityMetadatas
            ...
            var parentRelationColumns = parentPrimaryColumns.map(function (parentPrimaryColumn) {
                ...
                column.build(_this.connection.namingStrategy);
                return column;
            });
    ...
    EntityMetadataBuilder.prototype.computeEntityMetadata = function (entityMetadata) {
        var _this = this;
        entityMetadata.embeddeds.forEach(function (embedded) { return embedded.build(_this.connection.namingStrategy); });
        entityMetadata.embeddeds.forEach(function (embedded) {
            embedded.columnsFromTree.forEach(function (column) { return column.build(_this.connection.namingStrategy); });
            embedded.relationsFromTree.forEach(function (relation) { return relation.build(); });
        });
        entityMetadata.ownColumns.forEach(function (column) { return column.build(_this.connection.namingStrategy); });
