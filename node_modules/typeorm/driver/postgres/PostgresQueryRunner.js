...
var PostgresQueryRunner = (function () {
    ...
    PostgresQueryRunner.prototype.connect = function () {
        ...
        this.databaseConnectionPromise = new Promise(function (ok, fail) {
            _this.driver.pool.connect(function (err, connection, release) {
                if (err)
                    return fail(err);
                _this.driver.connectedQueryRunners.push(_this);
                _this.databaseConnection = connection;
                _this.releaseCallback = release;
                connection.query("SET search_path TO '" + _this.schemaName + "', 'public';", function (err) {
                    if (err) {
                        _this.driver.connection.logger.logFailedQuery("SET search_path TO '" + _this.schemaName + "', 'public';", [], _this);
