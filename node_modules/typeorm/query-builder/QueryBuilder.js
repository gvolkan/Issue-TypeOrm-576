...
var QueryBuilder = (function () {
     ...
     */
    QueryBuilder.prototype.getSqlAndParameters = function () {
     ...
     */
    QueryBuilder.prototype.setMainAlias = function (entityTarget, aliasName) {
        ...
        if (this.connection.hasMetadata(entityTarget)) {
            ...
            var subQuery = entityTarget instanceof Function ? entityTarget(this.subQuery()).getQuery() : entityTarget;
            this.expressionMap.createMainAlias({
                name: aliasName,
    ...
    QueryBuilder.prototype.replacePropertyNames = function (statement) {
        ...
        this.expressionMap.aliases.forEach(function (alias) {
            ...
            alias.metadata.columns.forEach(function (column) {
                ...
                statement = statement.replace(new RegExp(expression, "gm"), "$1" + _this.escape(alias.name) + "." + _this.escape(column.databaseName) + "$2");
                var expression2 = "([ =\(]|^.{0})" + alias.name + "\\." + column.propertyName + "([ =\)\,]|.{0}$)";
                statement = statement.replace(new RegExp(expression2, "gm"), "$1" + _this.escape(alias.name) + "." + _this.escape(column.databaseName) + "$2");
            });
            alias.metadata.relations.forEach(function (relation) {
                relation.joinColumns.concat(relation.inverseJoinColumns).forEach(function (joinColumn) {
                    ...
                    statement = statement.replace(new RegExp(expression, "gm"), "$1" + _this.escape(alias.name) + "." + _this.escape(joinColumn.databaseName) + "$2"); // todo: fix relation.joinColumns[0], what if multiple columns
                });
                if (relation.joinColumns.length > 0) {
                    var expression = "([ =\(]|^.{0})" + alias.name + "\\." + relation.propertyPath + "([ =\)\,]|.{0}$)";
                    statement = statement.replace(new RegExp(expression, "gm"), "$1" + _this.escape(alias.name) + "." + _this.escape(relation.joinColumns[0].databaseName) + "$2"); // todo: fix relation.joinColumns[0], what if multiple columns
                }
            });
        });
        return statement;
    ...
    QueryBuilder.prototype.createWhereIdsExpression = function (ids) {
        ...
        var whereStrings = ids.map(function (id, index) {
            ...
            metadata.primaryColumns.forEach(function (primaryColumn, secondIndex) {
                whereSubStrings.push(_this.escape(alias) + "." + _this.escape(primaryColumn.databaseName) + "=:id_" + index + "_" + secondIndex);
            ...
            metadata.parentIdColumns.forEach(function (parentIdColumn, secondIndex) {
                whereSubStrings.push(_this.escape(alias) + "." + _this.escape(parentIdColumn.databaseName) + "=:parentId_" + index + "_" + secondIndex);
